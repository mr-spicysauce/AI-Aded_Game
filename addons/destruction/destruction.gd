...
export var shard_scene : PackedScene = preload("res://game objects/Broken Glass objects.tscn")
...
func _ready():
	shards = DestructionUtils.create_shards(shard_scene.instance(), shard_template)
...
func _on_glass_thing_break_glass():
	destroy()
